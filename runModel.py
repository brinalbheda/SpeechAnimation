...
class runModel:
    ...
    def __init__(self):
         ...
         self.outputFolderBlack = outputFolderBlack
         ...
         if(os.path.exists(self.outputFolderBlack)):
             cmd = 'rm -rf '+self.outputFolderBlack + '&& mkdir ' + self.outputFolderBlack
             subprocess.call(cmd ,shell=True)
         if not(os.path.exists(self.outputFolderBlack)):
             subprocess.call('mkdir -p ' + self.outputFolderBlack, shell=True)  
    ...
    def predict_y(self,X):
        with open('data/pca/scalery22847.pickle', 'rb') as pkl_file: 
    ...
    def get_kp_data(self):
        with open('data/test_data_' + filename + '/kp_test.pickle', 'rb') as pkl_file: 
    ...
    def drawLips(self,keypoints, new_img, c, th, show):
    ...
    def get_mouth_boundary_images(self,y_pred,kp):
        for idx, (x, k) in enumerate(zip(y_pred, kp)):
            ...
            #For actual video
            imgfile = 'data/test_data_' + filename + '/images/' + str(idx+1).rjust(5, '0') + '.png'  
            im = cv2.imread(imgfile)
            self.drawLips(keypoints, im, c = (255, 0, 0), th = 1, show = False)
            ...
            #For keypoint video
            imgfile = 'data/black_data_' + filename + '/images/' + str(idx+1).rjust(5, '0') + '.png' 
            im = cv2.imread(imgfile)
            self.drawLips(keypoints, im, c = (255, 0, 0), th = 1, show = False)
            ...
            cv2.imwrite(self.outputFolderBlack + str(idx) + '.png', im1)
...
#Generate Keypoints video
os.chdir(outputFolderBlack)
with imageio.get_writer('Results.mp4', mode='I', fps=30) as writer:
        for file in sorted(glob.glob("*.png"), key=os.path.getmtime):
            image = imageio.imread(file)
            writer.append_data(image)
print("Keypoint Video without audio generated!!!!")


os.chdir('..')

input_path = outputFolderBlack + "Results.mp4"
result_path = outputFolderBlack + "Output.mp4"

cmd = "ffmpeg -i '"+input_path+ "' -i '"+ inputFile + "' \
-c:v copy -c:a aac -strict experimental '"+result_path+"'"
subprocess.call(cmd ,shell=True)
print("Keypoint Video with audio generated!!!")
